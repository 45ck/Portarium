...
describe('software change management reference pack artefacts', () => {
  it('parses manifest and all declared assets', async () => {
    ...
    expect(manifest.assets.complianceProfiles).toContain('compliance/scm-change-governance.json');
    ...
    const complianceProfile = await readJson(
      path.join(packRoot, 'compliance/scm-change-governance.json'),
    );
    ...
    const parsedComplianceProfile = parsePackComplianceProfileV1(complianceProfile);
    expect(parsedComplianceProfile.packId).toBe(manifest.id);
    expect(parsedComplianceProfile.profileId).toBe('scm.change-governance');
  });
});
