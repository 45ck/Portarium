/**
 * Failure injection integration tests for idempotency + outbox.
 *
 * Covers cascading and combined failure scenarios: transient command failures
 * followed by transient dispatcher failures, max-retry exhaustion, and ordering
 * guarantees when entries permanently fail.
 *
 * Split from idempotency-outbox-replay-safety.integration.test.ts to stay within max-lines.
 * Bead: bead-zmf3
 */
...
// ---------------------------------------------------------------------------
// In-memory fixtures (mirrored from replay-safety test for isolation)
// ---------------------------------------------------------------------------
