/**
 * H7: Companion approval CLI — Proxy extension
 *
 * Same REST approval store as H2, plus a GET /approvals endpoint
 * that lists all pending approvals for the companion CLI to discover.
 *
 * Endpoints:
 *   POST /tools/invoke        — intercepts CRITICAL tools, returns pending_approval
 *   GET  /approvals           — list all approvals (filterable by ?status=pending)
 ...
 * Imported API:    const { url, close } = await startApprovalProxy(9999);
 */
...
async function handleRequest(req, res, port) {
  ...
  if (req.method === 'GET' && url.pathname === '/health') {
    jsonResponse(res, 200, {
      status: 'ok',
      service: 'h7-companion-cli-approval-proxy',
      port,
  ...
  if (req.method === 'POST' && url.pathname === '/tools/invoke') {
    ...
    if (CRITICAL_TOOLS.has(toolName)) {
      ...
      console.log(`[h7-proxy] CRITICAL tool "${toolName}" — approval required: ${approvalId}`);
    ...
    jsonResponse(res, 200, {
      ...
      tool: toolName,
      output: {
        result: `[h7-proxy] executed ${toolName}`,
        parameters,
  ...
  // GET /approvals — list all approvals, optionally filtered by ?status=
  ...
  if (req.method === 'POST' && decideMatch) {
    ...
    console.log(`[h7-proxy] Approval ${id} → ${decision}`);
...
/**
 * Start the approval proxy.
 * @param {number} [port=9999]
 * @returns {Promise<{ url: string; close: () => void }>}
 */
export function startApprovalProxy(port = 9999) {
  return new Promise((resolve, reject) => {
    const server = createServer((req, res) => {
      handleRequest(req, res, port).catch((err) => {
        console.error('[h7-proxy] Unhandled error:', err);
        if (!res.headersSent) {
...
if (isMain) {
  ...
  startApprovalProxy(port).catch((err) => {
    console.error('[h7-proxy] Failed to start:', err);
    process.exit(1);
  });
}
