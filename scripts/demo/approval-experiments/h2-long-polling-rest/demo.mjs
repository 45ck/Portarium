/**
 * H2: Long-polling REST — Demo script
 *
 * Simulates the full approval flow:
 *   1. Start the approval proxy
 *   2. Invoke a CRITICAL tool → receive pending approvalId
 *   3. Poll for approval status (long-polling plugin)
 *   4. Auto-approve (if DEMO_AUTO_APPROVE=1) or wait for external decision
 *   5. Show final result
 *
...
async function main() {
  log('=== H2: Long-polling REST approval-wait loop ===');
  log('');
...
main().catch((err) => {
  console.error('[h2-demo] Fatal error:', err);
  process.exit(1);
});
