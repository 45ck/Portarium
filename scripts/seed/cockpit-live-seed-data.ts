...
export const COCKPIT_LIVE_SEED_IDS = {
  ...
  machines: ['machine-edge-001', 'machine-openclaw-gateway-demo'],
  agents: [
    'agent-finance-001',
    'agent-iam-001',
    'agent-robotics-001',
    'agent-openclaw-gateway-demo',
  ],
...
export function createCockpitLiveSeedBundle(
  ...
  const machines = [
    ...
    parseMachineRegistrationV1({
      ...
      workspaceId: workspaceIdRaw,
      endpointUrl: 'https://openclaw-gateway.local.invalid/v1',
      active: true,
      displayName: 'OpenClaw gateway demo',
      capabilities: ['read:external', 'agent:analyze', 'agent:generate', 'machine:invoke'],
      registeredAtIso: '2026-06-13T00:00:00.000Z',
      executionPolicy: {
        isolationMode: 'PerTenantWorker',
        egressAllowlist: ['https://openclaw-gateway.local.invalid'],
        workloadIdentity: 'Required',
      },
      authConfig: { kind: 'bearer', secretRef: 'grants/cg-live-machine' },
    }),
  ...
  const agents = [
    ...
    parseAgentConfigV1({
      ...
      capabilities: ['read:external', 'agent:analyze', 'agent:generate', 'machine:invoke'],
      policyTier: 'HumanApprove',
      allowedTools: ['read:external', 'agent:analyze', 'agent:generate', 'machine:invoke'],
      registeredAtIso: '2026-06-13T00:00:10.000Z',
      operatorUi: {
        schemaVersion: 1,
