...
async function renderRoute() {
  const router = createCockpitRouter({
    history: createMemoryHistory({ initialEntries: ['/engineering/evidence-cards/static'] }),
  });
...
describe('Static engineering evidence cards route', () => {
  it('renders checked-in GSLR cards without live engineering data endpoint calls', async () => {
    await renderRoute();

    expect(
      await screen.findByRole('heading', { name: 'Static Engineering Evidence Cards' }),
    ).toBeTruthy();
    ...
    expect(screen.getByText('No live ingestion')).toBeTruthy();
