...
const statusColor: Record<string, string> = {
  ...
  unhealthy: 'text-red-600 bg-red-50 dark:text-red-400 dark:bg-red-950',
};

function AdaptersPage() {
  const { activeWorkspaceId: wsId } = useUIStore();
  const { data, isLoading, isError, refetch } = useAdapters(wsId);
  const adapters = data?.items ?? [];

  const columns = [
    {
      key: 'name',
      header: 'Name',
      render: (row: AdapterSummary) => <span className="font-medium">{row.name}</span>,
    },
    {
      key: 'adapterId',
      header: 'Adapter ID',
      render: (row: AdapterSummary) => (
    ...
    },
    {
      ...
      header: 'SoR Family',
      width: '120px',
      render: (row: AdapterSummary) => <Badge variant="secondary">{row.sorFamily}</Badge>,
    },
    {
      key: 'status',
      header: 'Status',
