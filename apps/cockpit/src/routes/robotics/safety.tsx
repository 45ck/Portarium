...
function EnforcementBadge({ mode }: { mode: EnforcementMode }) {
  ...
  return <Badge variant="outline" className={cn('text-[11px]', c.className)} aria-label={`Enforcement: ${mode}`}>{c.label}</Badge>
}
  ...
  return (
    <div className="p-6 space-y-8">
      <div className="flex items-start justify-between gap-4">
        <PageHeader title="Safety & E-Stop" description="Global safety controls, constraints, and audit trail" />
        <Button variant="destructive" size="sm" className="shrink-0 gap-2 font-semibold" disabled={globalEstopActive} onClick={() => setShowEstopModal(true)} aria-label="Activate global E-Stop for all robots">
          <ShieldAlert className="h-4 w-4" />
          {globalEstopActive ? 'E-Stop Active' : 'Global E-Stop'}
        </Button>
      </div>

      <div role="status" aria-live="polite" className={cn('rounded-md border px-4 py-3 flex items-center gap-3 text-sm font-medium',
        globalEstopActive ? 'bg-red-50 border-red-300 text-red-800' : 'bg-green-50 border-green-200 text-green-800')}>
        {globalEstopActive ? (
          <><ShieldAlert className="h-4 w-4 text-red-600" /><span>⚠ GLOBAL E-STOP ACTIVE — All robots halted</span>
          <Button variant="outline" size="sm" className="ml-auto h-7 text-xs" onClick={() => setShowClearModal(true)}>Clear E-Stop (admin)</Button></>
        ) : (
          <><CheckCircle2 className="h-4 w-4 text-green-600" /><span>System NOMINAL — {constraints.reduce((n, c) => n + c.robotCount, 0)} robots monitored, 0 in E-Stop state</span></>
        )}
      </div>
      ...
      <section>
              ...
              <tbody>
                {constraints.map((sc) => (
                  <tr key={sc.constraintId} className="border-t border-border hover:bg-muted/20">
                    ...
                    <td className="px-3 py-2"><EnforcementBadge mode={sc.enforcement} /></td>
                    <td className="px-3 py-2 text-center">{sc.robotCount}</td>
                    <td className="px-3 py-2"><Button variant="outline" size="sm" className="h-6 text-xs" aria-label={`Edit constraint for ${sc.site}`}>Edit</Button></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Button variant="outline" size="sm" className="mt-2 h-7 text-xs">+ Add Constraint</Button>
      </section>
              ...
              <tbody>
                {thresholds.map((t) => (
                  <tr key={t.actionClass} className="border-t border-border hover:bg-muted/20">
                    ...
                    <td className="px-3 py-2"><TierBadge tier={t.tier} /></td>
      ...
      <Dialog open={showEstopModal} onOpenChange={setShowEstopModal}>
        <DialogContent>
          <DialogHeader><DialogTitle className="flex items-center gap-2 text-destructive"><AlertTriangle className="h-5 w-5" />Confirm Global E-Stop</DialogTitle></DialogHeader>
      ...
      <Dialog open={showClearModal} onOpenChange={setShowClearModal}>
        <DialogContent>
          <DialogHeader><DialogTitle>Clear Global E-Stop</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">Provide a rationale for clearing the E-Stop. This is required and will be logged.</p>
            <Textarea aria-required="true" placeholder="Rationale for clearing E-Stop..." value={clearRationale} onChange={(e) => setClearRationale(e.target.value)} className="min-h-[80px] resize-none text-sm" />
          </div>
          <DialogFooter>
