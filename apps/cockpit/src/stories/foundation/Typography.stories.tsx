...
const scale = [
  { label: 'Display',     className: 'text-3xl font-bold tracking-tight',    sample: 'Dashboard Overview' },
  ...
  { label: 'Body',        className: 'text-sm',                               sample: 'The workflow completed all 14 steps without intervention.' },
  { label: 'Body Small',  className: 'text-xs',                               sample: 'Last updated 3 minutes ago by system agent.' },
  ...
  return (
    <div className="bg-background p-6 space-y-6">
      {scale.map(({ label, className, sample }) => (
        <div key={label} className="flex items-baseline gap-4 border-b border-border pb-4 last:border-0">
