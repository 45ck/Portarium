...
async function postApprovalDecision(wsId: string, id: string, body: ApprovalDecisionRequest): Promise<ApprovalSummary> {
...
export function useApprovalDecision(wsId: string, id: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['approvals', wsId] }) },
  })
}
