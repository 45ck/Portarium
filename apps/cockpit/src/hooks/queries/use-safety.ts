...
export function useSafetyConstraints(wsId: string) {
  return useQuery({ queryKey: ['safety-constraints', wsId], queryFn: () => fetchConstraints(wsId) })
}

export function useApprovalThresholds(wsId: string) {
  return useQuery({ queryKey: ['approval-thresholds', wsId], queryFn: () => fetchThresholds(wsId) })
}
