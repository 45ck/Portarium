...
export function useWorkforceMembers(wsId: string) {
  return useQuery({ queryKey: ['workforce-members', wsId], queryFn: () => fetchWorkforceMembers(wsId) })
}

export function useWorkforceQueues(wsId: string) {
  return useQuery({ queryKey: ['workforce-queues', wsId], queryFn: () => fetchWorkforceQueues(wsId) })
}
