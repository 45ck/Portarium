...
import * as OpenClaw from './openclaw-demo';
...
const OPENCLAW_POLICY_STATUSES = ['active', 'draft', 'archived'] as const;
const OPENCLAW_SOD_TYPES = ['simple', 'n-of-m', 'blocked-role'] as const;
...
describe('Cockpit fixture parity', () => {
  ...
  it('keeps OpenClaw fixture exports aligned with Cockpit DTO assumptions', () => {
    expectNonEmptyArray('OpenClaw.WORK_ITEMS', OpenClaw.WORK_ITEMS);
    expectUniqueIds('OpenClaw.WORK_ITEMS', OpenClaw.WORK_ITEMS, 'workItemId');
    OpenClaw.WORK_ITEMS.forEach((item, index) => {
      const record = asRecord(item);
      expectRequiredKeys(`OpenClaw.WORK_ITEMS[${index}]`, record, [
        'schemaVersion',
        'workItemId',
        'workspaceId',
        'createdAtIso',
      ...
      expectEnum(`OpenClaw.WORK_ITEMS[${index}].status`, record.status, WORK_ITEM_STATUSES);
      expectIso(`OpenClaw.WORK_ITEMS[${index}].createdAtIso`, record.createdAtIso);
    });

    expectNonEmptyArray('OpenClaw.RUNS', OpenClaw.RUNS);
    expectUniqueIds('OpenClaw.RUNS', OpenClaw.RUNS, 'runId');
    OpenClaw.RUNS.forEach((run, index) => {
      const record = asRecord(run);
      expectRequiredKeys(`OpenClaw.RUNS[${index}]`, record, [
        'schemaVersion',
        'runId',
        'workspaceId',
        'workflowId',
      ...
      expectEnum(`OpenClaw.RUNS[${index}].status`, record.status, RUN_STATUSES);
      expectEnum(`OpenClaw.RUNS[${index}].executionTier`, record.executionTier, EXECUTION_TIERS);
      expectIso(`OpenClaw.RUNS[${index}].createdAtIso`, record.createdAtIso);
    });

    expectNonEmptyArray('OpenClaw.APPROVALS', OpenClaw.APPROVALS);
    expectUniqueIds('OpenClaw.APPROVALS', OpenClaw.APPROVALS, 'approvalId');
    OpenClaw.APPROVALS.forEach((approval, index) => {
      const record = asRecord(approval);
      expectRequiredKeys(`OpenClaw.APPROVALS[${index}]`, record, [
        'schemaVersion',
        'approvalId',
        'workspaceId',
        'runId',
      ...
      expectEnum(`OpenClaw.APPROVALS[${index}].status`, record.status, APPROVAL_STATUSES);
      expectIso(`OpenClaw.APPROVALS[${index}].requestedAtIso`, record.requestedAtIso);
      if (record.sodEvaluation !== undefined) {
        expectSodEvaluation(`OpenClaw.APPROVALS[${index}].sodEvaluation`, record.sodEvaluation);
      }
      if (record.policyRule !== undefined) {
        expectPolicyRule(`OpenClaw.APPROVALS[${index}].policyRule`, record.policyRule);
      }
    });

    expectNonEmptyArray('OpenClaw.PLANS', OpenClaw.PLANS);
    expectUniqueIds('OpenClaw.PLANS', OpenClaw.PLANS, 'planId');
    OpenClaw.PLANS.forEach((plan, index) => {
      const record = asRecord(plan);
      expectRequiredKeys(`OpenClaw.PLANS[${index}]`, record, [
        'schemaVersion',
        'planId',
        'workspaceId',
        'createdAtIso',
      ...
      expectIso(`OpenClaw.PLANS[${index}].createdAtIso`, record.createdAtIso);
      expect(Array.isArray(record.plannedEffects)).toBe(true);
      (record.plannedEffects as unknown[]).forEach((effect, effectIndex) => {
        expectPlanEffect(`OpenClaw.PLANS[${index}].plannedEffects[${effectIndex}]`, effect);
      });
    });

    expectNonEmptyArray('OpenClaw.EVIDENCE', OpenClaw.EVIDENCE);
    expectUniqueIds('OpenClaw.EVIDENCE', OpenClaw.EVIDENCE, 'evidenceId');
    OpenClaw.EVIDENCE.forEach((entry, index) => {
      const record = asRecord(entry);
      expectRequiredKeys(`OpenClaw.EVIDENCE[${index}]`, record, [
        'schemaVersion',
        'evidenceId',
        'workspaceId',
        'occurredAtIso',
      ...
      expectEnum(`OpenClaw.EVIDENCE[${index}].category`, record.category, EVIDENCE_CATEGORIES);
      expectEnum(
        `OpenClaw.EVIDENCE[${index}].actor.kind`,
        asRecord(record.actor).kind,
        EVIDENCE_ACTORS,
      );
      expectIso(`OpenClaw.EVIDENCE[${index}].occurredAtIso`, record.occurredAtIso);
    });

    expectNonEmptyArray('OpenClaw.AGENTS', OpenClaw.AGENTS);
    expectUniqueIds('OpenClaw.AGENTS', OpenClaw.AGENTS, 'agentId');
    OpenClaw.AGENTS.forEach((agent, index) => {
      const record = asRecord(agent);
      expectRequiredKeys(`OpenClaw.AGENTS[${index}]`, record, [
        'schemaVersion',
        'agentId',
        'workspaceId',
        'name',
      ...
      expectCapabilities(
        `OpenClaw.AGENTS[${index}].allowedCapabilities`,
        record.allowedCapabilities,
      );
      if (record.policyTier !== undefined) {
        expectEnum(`OpenClaw.AGENTS[${index}].policyTier`, record.policyTier, EXECUTION_TIERS);
      }
    });

    expectNonEmptyArray('OpenClaw.POLICIES', OpenClaw.POLICIES);
    expectUniqueIds('OpenClaw.POLICIES', OpenClaw.POLICIES, 'policyId');
    OpenClaw.POLICIES.forEach((policy, index) => {
      const record = asRecord(policy);
      expectExactKeys(`OpenClaw.POLICIES[${index}]`, record, [
        'schemaVersion',
        'policyId',
        'name',
        'description',
      ...
      expectRequiredKeys(`OpenClaw.POLICIES[${index}]`, record, [
        'schemaVersion',
        'policyId',
        'name',
        'description',
      ...
      expectEnum(`OpenClaw.POLICIES[${index}].tier`, record.tier, EXECUTION_TIERS);
      expectEnum(
        `OpenClaw.POLICIES[${index}].irreversibility`,
        record.irreversibility,
        POLICY_IRREVERSIBILITY,
      );
      expectEnum(`OpenClaw.POLICIES[${index}].status`, record.status, OPENCLAW_POLICY_STATUSES);
      expectIso(`OpenClaw.POLICIES[${index}].createdAtIso`, record.createdAtIso);
      expectIso(`OpenClaw.POLICIES[${index}].updatedAtIso`, record.updatedAtIso);
      if (record.sodRule !== undefined) {
        const sodRule = asRecord(record.sodRule);
        expectExactKeys(`OpenClaw.POLICIES[${index}].sodRule`, sodRule, [
          'type',
          'rolesRequired',
          'nRequired',
          'nTotal',
        ]);
        expectRequiredKeys(`OpenClaw.POLICIES[${index}].sodRule`, sodRule, [
          'type',
          'rolesRequired',
        ]);
        expectEnum(`OpenClaw.POLICIES[${index}].sodRule.type`, sodRule.type, OPENCLAW_SOD_TYPES);
        asStringArray(sodRule.rolesRequired);
      }
    });

    expectNonEmptyArray('OpenClaw.TOOL_CLASSIFICATIONS', OpenClaw.TOOL_CLASSIFICATIONS);
    expectUniqueIds('OpenClaw.TOOL_CLASSIFICATIONS', OpenClaw.TOOL_CLASSIFICATIONS, 'toolName');
    OpenClaw.TOOL_CLASSIFICATIONS.forEach((tool, index) => {
      const record = asRecord(tool);
      expectExactKeys(`OpenClaw.TOOL_CLASSIFICATIONS[${index}]`, record, [
        'schemaVersion',
        'toolName',
        'category',
        'minimumTier',
      ...
      expectRequiredKeys(`OpenClaw.TOOL_CLASSIFICATIONS[${index}]`, record, [
        'schemaVersion',
        'toolName',
        'category',
        'minimumTier',
      ...
      expectEnum(
        `OpenClaw.TOOL_CLASSIFICATIONS[${index}].category`,
        record.category,
        TOOL_CATEGORIES,
      );
      expectEnum(
        `OpenClaw.TOOL_CLASSIFICATIONS[${index}].minimumTier`,
        record.minimumTier,
        EXECUTION_TIERS,
      );
    });
