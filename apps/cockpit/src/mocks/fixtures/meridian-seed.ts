...
const ADAPTER_DEFS = [
  ...
  {
    id: 'adapter-fda-reporting',
    name: 'FDA Reporting (custom)',
    sorFamily: 'RegulatoryCompliance',
  },
...
const AGENT_DEFS = [
  {
    id: 'agent-order-router',
    name: 'Order Router',
    model: 'claude-opus-4-6',
    caps: ['read:external', 'analyze', 'classify'] as const,
    wfIds: ['wf-order-fulfillment'],
  },
  {
    id: 'agent-deviation-detector',
    name: 'Deviation Detector',
    model: 'claude-sonnet-4-6',
    caps: ['read:external', 'analyze', 'notify'] as const,
    wfIds: ['wf-cold-chain-deviation', 'wf-incident-report'],
  },
  {
    id: 'agent-compliance-checker',
    name: 'Compliance Checker',
    model: 'claude-opus-4-6',
    caps: ['read:external', 'analyze', 'classify', 'generate'] as const,
    wfIds: ['wf-controlled-substance-audit', 'wf-qc-approval'],
  },
  {
    id: 'agent-maintenance-scheduler',
    name: 'Maintenance Scheduler',
    model: 'claude-haiku-4-5-20251001',
    caps: ['read:external', 'analyze'] as const,
    wfIds: ['wf-maintenance-window'],
  },
  {
    id: 'agent-invoice-reconciler',
    name: 'Invoice Reconciler',
    model: 'claude-sonnet-4-6',
    caps: ['read:external', 'analyze', 'classify'] as const,
    wfIds: ['wf-supplier-invoice'],
  },
  {
    id: 'agent-fda-reporter',
    name: 'FDA Report Generator',
    model: 'claude-opus-4-6',
    caps: ['read:external', 'generate', 'write:external'] as const,
    wfIds: ['wf-controlled-substance-audit'],
  },
  {
    id: 'agent-cold-chain-analyst',
    name: 'Cold Chain Analyst',
    model: 'claude-sonnet-4-6',
    caps: ['read:external', 'analyze', 'notify'] as const,
    wfIds: ['wf-cold-chain-deviation'],
  },
  {
    id: 'agent-robot-coordinator',
    name: 'Robot Mission Coordinator',
    model: 'claude-haiku-4-5-20251001',
    caps: ['read:external', 'analyze', 'execute-code'] as const,
    wfIds: ['wf-order-fulfillment', 'wf-maintenance-window'],
  },
  {
    id: 'agent-audit-trail',
    name: 'Audit Trail Compiler',
    model: 'claude-sonnet-4-6',
    caps: ['read:external', 'generate'] as const,
    wfIds: ['wf-controlled-substance-audit', 'wf-qc-approval'],
  },
  {
    id: 'agent-sla-watchdog',
    name: 'SLA Watchdog',
    model: 'claude-haiku-4-5-20251001',
    caps: ['read:external', 'analyze', 'notify'] as const,
    wfIds: ['wf-order-fulfillment', 'wf-supplier-invoice'],
  },
] as const;
...
const MEMBER_DEFS = [
  ...
  {
    id: 'wfm-m002',
    userId: 'user-james-walker',
    name: 'James Walker',
    role: 'Warehouse Ops Lead — Chicago',
    caps: ['operations.dispatch', 'operations.approval'] as const,
  },
  {
    id: 'wfm-m003',
    userId: 'user-priya-patel',
    name: 'Priya Patel',
    role: 'QC Manager — Chicago',
    caps: ['operations.approval', 'operations.escalation'] as const,
  },
  {
    id: 'wfm-m004',
    userId: 'user-derek-johnson',
    name: 'Derek Johnson',
    role: 'Robotics Supervisor — Chicago',
    caps: ['operations.dispatch', 'robotics.supervision', 'robotics.safety.override'] as const,
  },
  {
    id: 'wfm-m005',
    userId: 'user-sarah-kim',
    name: 'Sarah Kim',
    role: 'Site Director — Dallas',
    caps: [
      'operations.dispatch',
      'operations.approval',
      'operations.escalation',
  ...
  {
    id: 'wfm-m006',
    userId: 'user-carlos-reyes',
    name: 'Carlos Reyes',
    role: 'Cold Chain Specialist — Dallas',
    caps: ['operations.dispatch', 'operations.approval'] as const,
  },
  {
    id: 'wfm-m007',
    userId: 'user-amanda-foster',
    name: 'Amanda Foster',
  ...
  {
    id: 'wfm-m008',
    userId: 'user-kevin-thompson',
    name: 'Kevin Thompson',
    role: 'Site Director — Atlanta',
    caps: ['operations.dispatch', 'operations.approval', 'operations.escalation'] as const,
  },
  {
    id: 'wfm-m009',
    userId: 'user-lisa-nguyen',
    name: 'Lisa Nguyen',
    role: 'Warehouse Ops Lead — Dallas',
    caps: ['operations.dispatch', 'operations.approval'] as const,
  },
  {
    id: 'wfm-m010',
    userId: 'user-robert-garcia',
    name: 'Robert Garcia',
    role: 'Robotics Supervisor — Dallas',
    caps: ['operations.dispatch', 'robotics.supervision', 'robotics.safety.override'] as const,
  },
  {
    id: 'wfm-m011',
    ...
    name: 'Emily Davis',
    role: 'Warehouse Ops Lead — Atlanta',
    caps: ['operations.dispatch', 'operations.approval'] as const,
  },
  {
    id: 'wfm-m012',
    ...
    caps: ['operations.dispatch', 'operations.escalation'] as const,
  },
] as const;
...
const QUEUE_DEFS = [
  {
    ...
    caps: ['operations.dispatch'] as const,
  ...
  {
    id: 'queue-compliance',
    name: 'Compliance & QC Approvals',
    caps: ['operations.approval'] as const,
    strategy: 'least-busy' as const,
    site: -1,
  },
  {
    id: 'queue-robotics',
    name: 'Robotics Supervision',
    caps: ['robotics.supervision'] as const,
    strategy: 'least-busy' as const,
    site: -1,
  },
  {
    id: 'queue-escalation',
    name: 'Escalation & Incidents',
    caps: ['operations.escalation'] as const,
...
export function generateMeridianDataset(cfg: MeridianDatasetConfig): MeridianDataset {
  ...
  const ADAPTERS = adapterSlice.map((a) => ({
    ...
    status: (rand() < 0.8
      ...
      : rand() < 0.5
        ? 'degraded'
        : 'unhealthy') as AdapterSummary['status'],
  ...
  const WORKFORCE_QUEUES: WorkforceQueueSummary[] = queueSlice.map((q) => ({
    ...
    memberIds: WORKFORCE_MEMBERS.filter((m) => m.queueMemberships.includes(q.id)).map(
      (m) => m.workforceMemberId,
    ),
  ...
  const robotStatusWeights = [0.6, 0.15, 0.1, 0.15];
  for (let i = 0; i < cfg.robotCount; i++) {
    ...
    ROBOTS.push({
      ...
      lastHeartbeatSec:
        status === 'Offline' ? Math.floor(rand() * 600 + 60) : Math.floor(rand() * 30 + 1),
      ...
      capabilities: ROBOT_CAPABILITIES[cls]!.slice(
        0,
        2 + Math.floor(rand() * (ROBOT_CAPABILITIES[cls]!.length - 1)),
      ),
    });
  }
  ...
  for (let i = 0; i < cfg.runCount; i++) {
    ...
    const started =
      status !== 'Pending' ? addMinutes(created, Math.floor(rand() * 3 + 1)) : undefined;
    const ended =
  ...
  for (let i = 0; i < cfg.workItemCount; i++) {
    ...
    const created = linkedRuns.reduce(
      (min, r) => (r.createdAtIso < min ? r.createdAtIso : min),
      linkedRuns[0]!.createdAtIso,
    );
    const isOpen = rand() < 0.6;
    const dueDate =
      rand() < 0.7 ? addMinutes(created, Math.floor(rand() * 4320 + 1440)) : undefined;
  ...
  const approverIds = memberSlice
    .filter((m) => (m.caps as readonly string[]).includes('operations.approval'))
    .map((m) => m.userId);
  for (let i = 0; i < cfg.approvalCount; i++) {
    ...
    const decided =
      status !== 'Pending' ? addMinutes(requested, Math.floor(rand() * 480 + 5)) : undefined;
    ...
    'wf-order-fulfillment': [
      {
        ...
        summary: 'Update sales order status to Released',
      },
      {
        operation: 'Create',
        sorName: 'adapter-ups-freight',
        portFamily: 'LogisticsShipping',
        externalType: 'ShipmentLabel',
        summary: 'Generate cold-chain shipment label',
      },
    ],
    'wf-qc-approval': [
      {
        operation: 'Update',
        sorName: 'LabWare LIMS',
        portFamily: 'LaboratoryInfo',
        externalType: 'QCRecord',
        summary: 'Record QC approval decision',
      },
      {
        operation: 'Create',
        sorName: 'Paperless-ngx',
        portFamily: 'DocumentManagement',
        externalType: 'CertificateOfAnalysis',
        summary: 'Generate certificate of analysis PDF',
      },
    ],
    'wf-maintenance-window': [
      {
        ...
        sorName: 'ServiceNow',
        portFamily: 'Itsm',
        externalType: 'ChangeRequest',
        summary: 'Close maintenance change request',
      },
      {
        operation: 'Update',
        sorName: 'SAP',
        portFamily: 'FinanceAccounting',
        externalType: 'MaintenanceOrder',
        summary: 'Record maintenance labour and parts cost',
      },
    ],
    'wf-incident-report': [
      {
        ...
        sorName: 'ServiceNow',
        portFamily: 'Itsm',
        externalType: 'IncidentRecord',
        summary: 'Create incident record with RCA',
      },
      {
        operation: 'Update',
        sorName: 'FDA Reporting',
        portFamily: 'RegulatoryCompliance',
        externalType: 'DeviationReport',
        summary: 'Submit deviation report to regulatory system',
      },
    ],
    'wf-supplier-invoice': [
      {
        ...
        sorName: 'SAP',
        portFamily: 'FinanceAccounting',
        externalType: 'InvoicePayment',
        summary: 'Schedule supplier invoice payment',
      },
      {
        operation: 'Update',
        sorName: 'SAP',
        ...
        summary: 'Mark purchase order as invoiced',
      },
    ],
    'wf-cold-chain-deviation': [
      {
        ...
        sorName: 'LabWare LIMS',
        portFamily: 'LaboratoryInfo',
        externalType: 'DispositionDecision',
        summary: 'Record disposition decision for affected shipment',
      },
      {
        operation: 'Update',
        sorName: 'SAP',
        ...
        summary: 'Flag delivery order as quarantined',
      },
    ],
    'wf-controlled-substance-audit': [
      {
        ...
        sorName: 'DEA ARCOS',
        portFamily: 'RegulatoryCompliance',
        externalType: 'AuditReport',
        summary: 'Submit Schedule II inventory audit to DEA ARCOS',
      },
      {
        operation: 'Update',
        sorName: 'LabWare LIMS',
        portFamily: 'LaboratoryInfo',
        externalType: 'ChainOfCustody',
        summary: 'Update chain-of-custody record',
      },
    ],
  };
  const DEFAULT_EFFECT_TEMPLATES: EffectTemplate[] = [
    {
      ...
      summary: 'Complete workflow step and close work order',
    },
  ];
  ...
  const PLANS: Plan[] = APPROVALS.map((approval, i) => {
    ...
    const templates =
      (run ? PLAN_EFFECT_TEMPLATES[run.workflowId] : null) ?? DEFAULT_EFFECT_TEMPLATES;
  ...
  for (let i = 0; i < cfg.evidenceCount; i++) {
    ...
    const actor: EvidenceActor = (() => {
      ...
      if (r < 0.5) return { kind: 'User' as const, userId: pick(rand, userIds) };
  ...
  const missionStatusWeights = [0.1, 0.2, 0.45, 0.15, 0.1];
  for (let i = 0; i < cfg.missionCount; i++) {
    ...
    const completed =
      (status === 'Completed' || status === 'Failed' || status === 'Cancelled') && dispatched
  ...
  const SAFETY_CONSTRAINTS: SafetyConstraint[] = [
    {
      constraintId: 'sc-m001',
      site: 'Chicago — Cold Room A',
      constraint: 'max_speed ≤ 0.3 m/s',
      enforcement: 'block',
      robotCount: Math.min(cfg.robotCount, 8),
    },
    {
      constraintId: 'sc-m002',
      site: 'Chicago — Loading Dock',
      constraint: 'no_autonomous_lift',
      enforcement: 'warn',
      robotCount: Math.min(cfg.robotCount, 4),
    },
    {
      constraintId: 'sc-m003',
      site: 'Dallas — Freezer Zone',
      constraint: 'max_speed ≤ 0.2 m/s',
      enforcement: 'block',
      robotCount: Math.min(cfg.robotCount, 6),
    },
    {
      constraintId: 'sc-m004',
      site: 'Dallas — Controlled Substance Vault',
      constraint: 'dual_operator_required',
      enforcement: 'block',
      robotCount: 2,
    },
    {
      constraintId: 'sc-m005',
      site: 'Atlanta — Warehouse Floor',
      constraint: 'max_speed ≤ 0.5 m/s',
      enforcement: 'warn',
      robotCount: Math.min(cfg.robotCount, 10),
    },
    {
      constraintId: 'sc-m006',
      site: 'All Sites — Outdoor',
      constraint: 'uav_flight_permit_required',
      enforcement: 'block',
      robotCount: Math.min(cfg.robotCount, 3),
    },
    {
      constraintId: 'sc-m007',
      site: 'All Sites',
      constraint: 'human_in_proximity ≤ 2m → pause',
      enforcement: 'block',
  ...
  const APPROVAL_THRESHOLDS: ApprovalThreshold[] = [
    ...
    {
      actionClass: 'pick (>5 kg)',
      tier: 'HumanApprove',
      notes: 'Heavy pharma containers require operator sign-off',
    },
    {
      actionClass: 'outdoor_flight',
      tier: 'HumanApprove',
      notes: 'Requires safety_admin sign-off per FAA Part 107',
    },
    {
      actionClass: 'controlled_substance_access',
      tier: 'ManualOnly',
      notes: 'DEA requires human-only handling',
    },
    {
      actionClass: 'cold_room_entry',
      tier: 'Assisted',
      notes: 'Robot may enter with operator monitoring',
    },
    {
      actionClass: 'forklift_pallet_lift',
      tier: 'HumanApprove',
      notes: 'AGV lift operations need supervisor confirmation',
    },
    {
      actionClass: 'temperature_override',
      tier: 'ManualOnly',
      notes: 'PLC temperature setpoint changes require dual approval',
    },
  ];
  ...
  const estopActors = [
    'safety_admin',
    'derek.johnson@meridian.com',
    'robert.garcia@meridian.com',
    'system-watchdog',
  ];
  ...
  const CREDENTIAL_GRANTS: CredentialGrantV1[] = adapterSlice.map((a, i) => {
    ...
    return {
      ...
      ...(isRevoked
        ? { revokedAtIso: addMinutes(issuedAt, Math.floor(rand() * 43200 + 1440)) }
        : {}),
    };
  });
  ...
  const avgSlaDays =
    slaDays.length > 0
      ? Math.round((slaDays.reduce((s, v) => s + v, 0) / slaDays.length) * 10) / 10
      : 2.0;
