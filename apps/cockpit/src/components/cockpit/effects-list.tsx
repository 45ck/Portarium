  ...
  return (
    <div className="flex items-center gap-2 py-1 text-xs">
      ...
      {confidence !== undefined && (
        <span className="text-[10px] text-muted-foreground shrink-0">{Math.round(confidence * 100)}%</span>
      )}
    </div>
  )
}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1">
      <h4 className="text-[11px] font-medium text-muted-foreground uppercase tracking-wide">{title}</h4>
      {children}
    </div>
  )
}
    ...
    <div className="space-y-3">
      ...
      {predicted && predicted.length > 0 && (
        <Section title="Predicted">
          {predicted.map((e) => <EffectRow key={e.effectId} effect={e} confidence={e.confidence} />)}
        </Section>
      )}
