...
- Each machine MUST support:
  - `POST /v1/invoke` with a JSON body (`MachineInvocationRequestV1`)
...
## `MachineInvocationRequestV1` (recommended shape)
...
The control plane accepts the historical `outputArtifactUris` alias when parsing machine invocation status payloads, but `artifactUris` is the canonical contract surface.
...
### `GET /v1/workspaces/{workspaceId}/machines/{machineId}/invocations/{invocationId}/stream`

Stream long-running machine invocation progress as server-sent events.

Event payloads follow `MachineInvocationProgressEventV1`:
