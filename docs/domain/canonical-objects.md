...
> The 16-member canonical object set with rationale, design principles, and evolution strategy.
...
3. **Branded IDs** -- Every canonical object has a branded ID type (`PartyId`, `TicketId`, `InvoiceId`, etc.) that prevents accidental mixing. These are defined in `src/domain/primitives/index.ts`.
...
| **Consent**           | `consent-v1.ts` (`ConsentV1`)                  | `consentId`, `tenantId`, `partyId`, `purpose`, `channel`, `optInStatus`, `capturedAtIso?`, `revokedAtIso?`, `suppressionEntries?`, `auditTrail?`, `externalRefs?`                                         |
...
| **Privacy Policy**    | `privacy-policy-v1.ts` (`PrivacyPolicyV1`)     | `privacyPolicyId`, `tenantId`, `name`, `versionLabel`, `effectiveFromIso`, `effectiveToIso?`, `defaultOptInStatus`, `suppressionListNames?`, `auditRetentionDays?`, `policyDocumentUrl?`, `externalRefs?` |
...
- `ConsentV1.partyId` anchors consent state to the unified `Party` identity while `PrivacyPolicyV1` captures the policy-level defaults that drive those consent records.
...
The table below shows the long-lived cross-port coverage for the canonical business objects that currently anchor port-family mapping work. Consent and privacy policy parsers are now first-class canonical contracts, but their adoption remains concentrated in privacy-sensitive marketing and governance flows rather than the broad port-family matrix below.
...
- **Merges** Customer + Lead + Vendor + Employee into **Party** as the unified actor identity.
- **Keeps** Ticket, Invoice, Payment, Task, Campaign, Asset, and Document as the long-lived backbone objects.
- **Adds** Subscription, Opportunity, Product, Order, Account, Consent, and Privacy Policy to cover recurring revenue, commercial workflows, and privacy-sensitive marketing operations.
- **Adds** ExternalObjectRef as the primitive escape hatch for SoR-specific data that should not be canonicalised.

**Net change**: 10 -> 16 runtime contracts (15 canonical entity parsers + 1 primitive). The Party unification removes the old duplicated actor types while the expanded set adds broader commercial and privacy coverage without abandoning the minimal-field design.
