...
When all of those are in place, autonomous backlog operation is mostly a matter of letting agents _also_ be actors on the same surface humans use — same Beads API, same approval gates, same evidence chain, same Cockpit. No new architecture; new actor types.
...
Engineering throughput becomes a function of policy permissiveness, not headcount. Humans become _more_ leveraged, not redundant — a human-with-Portarium does the work of a small team; a small team-with-Portarium does the work of a department.
...
- Rationale text input is _required_ on every approval (min 10 chars — already in `ux-layout.md`).
...
- Approval surfaces show the _full_ plan and _full_ diff, never summaries. Humans see exactly what they're approving.
...
These are not features. They are _shape decisions_ that cost almost nothing in v1 if known up-front, and cost a structural rewrite to add later. v1 must include all five.
...
v1 ships with humans as the only actors in the Cockpit. Phase 3 adds agent actors when the surrounding architecture is solid enough to govern them. The point of v1 is to _not preclude_ phase 3, not to ship phase 3.
