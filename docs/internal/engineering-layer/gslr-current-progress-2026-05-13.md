...
Status: post-GSLR-36 progress update
Tracking beads: `bead-1253`, `bead-1254`, `bead-1255`, `bead-1256`, `bead-1257`, `bead-1258`, `bead-1259`, `bead-1260`, `bead-1261`, `bead-1262`, `bead-1263`, `bead-1264`, `bead-1265`, `bead-1266`, `bead-1267`, `bead-1268`, `bead-1269`, `bead-1270`, `bead-1271`, `bead-1272`, `bead-1273`, `bead-1274`, `bead-1275`, `bead-1276`, `bead-1277`, `bead-1278`, `bead-1279`, `bead-1280`, `bead-1281`, `bead-1282`, `bead-1283`
...
prompt-language experiment evidence
  ...
  -> persistent static repository unbound adapter shell contract
  -> persistent static repository unbound adapter shell review
```
...
- a docs/test-only persistent static repository database-adapter review
  checkpoint that pauses at the contract harness by default, opens only a draft
  PostgreSQL adapter bead after explicit operator/product/engineering approval,
...
- a docs/test-only persistent static repository draft PostgreSQL adapter
  ...
  surfaces blocked.
- a docs/test-only persistent static repository draft SQL review packet that
  reviews table mapping, statement plan, constraints, rollback posture, and
  contract assertion coverage while blocking executable adapter code, production
  DDL/DML, connection config, runtime surfaces, actions, and MC connector
  access.
- a docs/test-only persistent static repository executable adapter design review
  that maps adapter interface, transaction boundaries, idempotent replay,
  conflict errors, contract harness binding, and observability while blocking
  executable code, connection config, generated SQL files, secrets, applied
  migrations, production tables/writes, runtime surfaces, actions, and MC
  connector access.
- a docs/test-only persistent static repository executable adapter scaffold that
  names the PostgreSQL adapter shell and port methods while keeping transaction
  boundaries unbound, database client binding absent, connection config absent,
  generated SQL files absent, secrets absent, migrations unapplied, reads/writes
  blocked, contract harness execution unbound, and runtime authority absent.
...
`bead-1276` proves database-adapter work can be governed before it starts:

- the GSLR-28 harness metadata gates any next adapter step;
- the default outcome pauses at the contract harness while review is requested;
- a draft PostgreSQL adapter bead opens only after operator, product, and
  engineering approval plus database-adapter value confirmation;
- decline is a valid `do-not-build-database-adapter` outcome;
- invalid harness metadata, production adapter scope, applied migrations,
...
`bead-1278` proves SQL review can be separated from executable adapter design:

- the review packet opens only from a ready GSLR-30 scaffold;
- table mapping, statement plan, constraint set, rollback posture, and contract
  assertion coverage must be reviewed;
- executable adapter code remains excluded;
- executable SQL, non-parameterized statements, raw payload columns, production
  DDL/DML, connection config, reviewer change requests, and runtime authority
  block.

`bead-1279` proves executable adapter design can be reviewed before any
executable adapter exists:

- the design review opens only from a ready GSLR-31 SQL review packet;
- adapter interface, transaction boundaries, idempotent replay, conflict error
  mapping, contract harness binding, and observability must be reviewed;
- implementation mode remains `design-review-only`;
- executable adapter code, connection config, generated SQL files, secret
  references, applied migrations, production tables/writes, reviewer change
  requests, and runtime authority block.

`bead-1280` proves an executable adapter shell can be reviewed before database
binding exists:

- the scaffold opens only from a ready GSLR-32 design review;
- the PostgreSQL adapter name and persistent repository port methods are fixed;
- transaction boundary remains planned-not-bound;
- database client binding, connection config, generated SQL files, secret
...
`bead-1282` proves the adapter shell can be made concrete without becoming a
database adapter:

- the shell opens only from ready GSLR-34 scaffold code-review evidence;
- the PostgreSQL adapter name, exported port methods, constructor boundary,
  transaction boundary, not-implemented method bodies, declared-only error
  mapping, and visible review TODOs are fixed;
- database client binding, connection config, generated SQL files, secrets,
  applied migrations, production tables, read/write execution, hidden review
  surfaces, and runtime authority block.

`bead-1283` proves a concrete unbound shell can be reviewed without authorizing
database implementation:

- the review opens only from a ready GSLR-35 unbound shell;
- port methods, constructor boundary, transaction boundary, not-implemented
  method bodies, declared-only error mapping, database-binding TODO, and
  contract-harness TODO must be reviewed;
- database client binding, connection config, generated SQL files, secrets,
  applied migrations, production tables, read/write execution, reviewer change
  requests, and runtime authority block.
...
The latest safe work item is database-binding design review. GSLR-20 supplied the
...
scaffold, `bead-1281` supplied the scaffold code-review evidence, and
`bead-1282` supplied the unbound adapter shell contract, and `bead-1283`
supplied the unbound adapter shell review.

See
[`static-evidence-review-workbench-plan-2026-05-13.md`](./static-evidence-review-workbench-plan-2026-05-13.md).

Proceed to `bead-1284`: database-binding design review. It may design the
future binding boundary but must still avoid database clients, connection
config, generated SQL files, secrets, applied migrations, production tables,
read/write execution, runtime cards, production actions, and MC
