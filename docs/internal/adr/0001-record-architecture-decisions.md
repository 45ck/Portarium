...
Each ADR is a short text file in `docs/internal/adr/`. The file name convention is `NNNN-title-with-dashes.md` where NNNN is a zero-padded sequence number.
...
- The existing decisions in `docs/internal/ADRs-v0.md` will be migrated into individual ADR files over time.
