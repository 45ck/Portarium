...
1. **Portarium never sends direct motor commands.** All platform-to-robot communication uses `IntentCommandV1` value objects that describe _what_ to do, not _how_ to do it.

2. **Local systems never override safety constraints.** If Portarium declares `maxVelocityMps: 1.5`, the local controller must enforce that limit. The local system may impose _stricter_ limits but never _looser_ ones.
